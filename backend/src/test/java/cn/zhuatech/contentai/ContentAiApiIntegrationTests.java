 ...
 */
@SpringBootTest @AutoConfigureMockMvc class ContentAiApiIntegrationTests {
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanReadShopfloorDashboard()throws Exception{mvc.perform(get("/api/shopfloor/dashboard").header("Authorization","Bearer "+operatorToken)).andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.metrics[0].label").value("计划内容数量"));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanSubmitProductionReport()throws Exception{mvc.perform(post("/api/shopfloor/work-orders/1/reports").header("Authorization","Bearer "+operatorToken).contentType(MediaType.APPLICATION_JSON).content("{\"operationName\":\"品牌审核\",\"goodQty\":2,\"defectQty\":1,\"remark\":\"表达一致\"}")).andExpect(status().isOk()).andExpect(jsonPath("$.message").value("反馈提交成功")).andExpect(jsonPath("$.data.completedQty").value(74));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanReviewContentBeforePublishing()throws Exception{mvc.perform(post("/api/shopfloor/content-review").header("Authorization","Bearer "+operatorToken).contentType(MediaType.APPLICATION_JSON).content("{\"content\":\"客户案例含手机号并使用禁用表述\",\"channel\":\"官网\",\"containsPersonalData\":true,\"prohibitedTerms\":[\"绝对保证\"],\"factualClaims\":3,\"citedClaims\":1}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.decision").value("BLOCK")).andExpect(jsonPath("$.data.riskScore").value(76)).andExpect(jsonPath("$.data.uncitedClaims").value(2)).andExpect(jsonPath("$.data.reasons.length()").value(3));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
