...
@Bean CommandLineRunner seed(OperatingUnitRepository units,WorkRecordRepository records,ResourceRegisterRepository resources,ReviewRecordRepository reviews,UserRepository users,PasswordEncoder encoder){return args->{if(units.count()>0)return;
 ...
 reviews.saveAll(List.of(new ReviewRecord("REV-260801-068",a,"品牌一致性",86,4,ReviewRecord.Result.PENDING,"闻岚"),new ReviewRecord("REV-260801-041",b,"渠道合规",48,1,ReviewRecord.Result.PASSED,"苏言"),new ReviewRecord("REV-260731-029",c,"事实准确性",24,6,ReviewRecord.Result.FAILED,"闻岚")));
 String demo=encoder.encode("Demo@2026"); users.saveAll(List.of(new UserAccount("operator",demo,"苏言",UserAccount.Role.DOMAIN_USER,"CNT-BRAND"),new UserAccount("planner",demo,"闻岚",UserAccount.Role.DOMAIN_OPERATOR,null),new UserAccount("quality",demo,"许禾",UserAccount.Role.QUALITY,null),new UserAccount("admin",encoder.encode("ZhuaTech@2026"),"系统管理员",UserAccount.Role.ADMIN,null)));};}}
